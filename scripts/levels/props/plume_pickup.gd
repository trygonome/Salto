...
extends Node3D
## Plume dorée posée au sommet d'un perchoir : elle tourne et flotte ; le héros qui l'atteint
## gagne des plumes, et elle ne revient pas avant la nuit suivante.

## Perchoir de la plume (indice dans le monde de la nuit).
var perch: int = 0

var _time: float = 0.0
var _taken: bool = false

## Une plume de cubes dorés qui s'affinent, et sa pointe arc-en-ciel.
const QUILL := 5
const QUILL_STEP := 0.8
const QUILL_COLOR := Vector3(2.13, 0.95, 0.6)
const QUILL_DARKEN := 0.03
...
func _ready() -> void:
	...
	for i: int in QUILL:
		VoxelMesh.add(cells, 0.0, i * QUILL_STEP, 0.0, Vector3(QUILL_COLOR.x, QUILL_COLOR.y, QUILL_COLOR.z - i * QUILL_DARKEN), QUILL_SIZE - i * QUILL_TAPER)
...
func _physics_process(delta: float) -> void:
	...
	if flat.length() > tuning.perch_pickup_radius or absf(hero.global_position.y + tuning.perch_pickup_height - global_position.y) > tuning.perch_pickup_height * 2.0:
		return
	_taken = true
	if not Game.profile.perch_taken.has(perch):
		Game.profile.perch_taken.append(perch)
	Game.add_plumes(tuning.perch_plumes)
	var fx: Effects = Effects.of(self)
	if fx:
		fx.burst(global_position, tuning.fx_plume_cubes, tuning.fx_plume_speed, tuning.fx_gold_hue)
		fx.word(GameTexts.WORD_PLUMES % tuning.perch_plumes, global_position + Vector3.UP * tuning.fx_plume_word_height, fx.gold, true)
