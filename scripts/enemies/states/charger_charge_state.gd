...
extends MuetState
## Cornu, charge : tout droit, vite, penché en avant, sur une distance fixe ; un gros coup pour
## qui est sur le chemin. S'il heurte un obstacle, il s'assomme (et devient plus fragile).
...
func enter(_previous: StringName) -> void:
	...
	_travelled = 0.0
	muet.face(direction)
	muet.body.rotation.y = muet.body.target_yaw
	var duration: float = tuning.charger_distance / tuning.charger_speed
	muet.strike(tuning.charger_radius, CombatMath.FULL_CIRCLE_DEG, direction, duration, muet.damage_of(&"damage"), true)
...
func physics_update(delta: float) -> void:
	...
	muet.body.set_motion(false, tuning.charger_lean, 0.0, false, false)
